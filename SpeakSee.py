...
def voice_interaction():
    """Handle voice interaction in a separate thread"""
    ...
    try:
        with sr.Microphone() as source:
            ...
            try:
                ...
                if 'YouTube' in query:
                    youtube_search(query.replace('YouTube', '').strip())
                else:
...
def start_voice_interaction(event=None):
    """Start or stop voice interaction"""
    global voice_interaction_enabled
...
def perform_search(query):
    """Perform Bing search"""
    if is_connected():
        webbrowser.open('https://www.bing.com?q=' + query.replace(' ', '+'))
...
def show_text_search():
    """Show the text search options window"""
    play_sound("menu")
    search_window = tk.Toplevel(root)
    search_window.title("SpeakSee-Type")
    search_window.geometry('350x250')
    center_window_on_parent(search_window, 350, 250)
    
    yt_frame = tk.Frame(search_window)
    yt_frame.pack(pady=5)
    tk.Label(yt_frame, text="YouTube Search:", font=("Helvetica", 10)).pack()
    yt_entry = tk.Entry(yt_frame, width=30, font=("Helvetica", 10))
    yt_entry.pack(pady=5)
    
    def search_yt(event=None):
        play_sound("menu")
        query = yt_entry.get()
        if query:
    ...
    yt_entry.bind("<Return>", search_yt)
    tk.Button(yt_frame, text="Search YouTube", command=search_yt, font=("Helvetica", 8)).pack(pady=5)
    
    bing_frame = tk.Frame(search_window)
    bing_frame.pack(pady=5)
    tk.Label(bing_frame, text="Bing Search:", font=("Helvetica", 10)).pack()
    bing_entry = tk.Entry(bing_frame, width=30, font=("Helvetica", 10))
    bing_entry.pack(pady=5)
    
    def search_bing(event=None):
        play_sound("menu")
        query = bing_entry.get()
        if query:
            perform_search(query)
            search_window.destroy()
    
    bing_entry.bind("<Return>", search_bing)
    tk.Button(bing_frame, text="Search Bing", command=search_bing, font=("Helvetica", 8)).pack(pady=5)
    
    tk.Button(search_window, text="Close", command=lambda: [play_sound("menu"), search_window.destroy()], 
...
menu_frame.place(x=-120, y=0, width=120, height=220)
...
tk.Frame(menu_frame, height=5, bg='#333333').pack(pady=(10, 0))
tk.Frame(menu_frame, height=5, bg='#333333').pack(pady=(0, 10))
...
if os.path.exists(image_path):
    try:
        ...
        button_label.bind("<Button-1>", start_voice_interaction)
...
active_indicator_top_right = tk.Label(root, width=2, height=1, bg='DodgerBlue', bd=2, relief='solid')
active_indicator_top_right.place(relx=1.0, rely=0, x=-25, y=10)
active_indicator_top_right.lower(listening_indicator_top_right)

active_indicator_bottom_right = tk.Label(root, width=2, height=1, bg='DodgerBlue', bd=2, relief='solid')
active_indicator_bottom_right.place(relx=1.0, rely=1.0, x=-25, y=-25)
active_indicator_bottom_right.lower(listening_indicator_bottom_right)

active_indicator_bottom_left = tk.Label(root, width=2, height=1, bg='DodgerBlue', bd=2, relief='solid')
